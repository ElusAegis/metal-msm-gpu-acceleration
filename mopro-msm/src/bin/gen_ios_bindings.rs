...
const LIBRARY_NAME: &str = "mopro_msm";
...
fn build_bindings(library_name: &str, mode: &str) {
    ...
    let framework_out = bindings_out.join("GPUMsmCore.xcframework");
    ...
    fs::rename(
        ...
        bindings_out.join("GPUMsmCore.swift"),
    )
    .expect("Failed to copy swift bindings file");

    // Rename the `ios_msmFFI.modulemap` file to `module.modulemap`
    fs::rename(
    ...
    let bindings_dest = manifest_dir.join("GPUMsmCoreBindings");
...
fn create_bindings_out_dir(base_dir: &Path) -> PathBuf {
    let bindings_out = base_dir.join("GPUMsmCoreBindings");
...
fn build_for_arch(arch: &str, build_dir: &Path, mode: &str) {
    ...
    build_cmd.arg("--features").arg("ios-bindings h2c");
